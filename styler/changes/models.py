...
@dataclass
class ChangeBatchPlan:
    """Plan legible de varios cambios que se ejecutan uno por uno.

    Cada ``ChangePlan`` conserva su DAG original. El lote solo decide el orden
    entre cambios y obliga a reconstruir cada plan justo antes de ejecutarlo,
    para que el estado producido por un cambio pueda reconciliar el siguiente.
    """
...
@dataclass(frozen=True)
class ChangeBatchProgressEvent:
    change_id: str
    change_name: str
    change_index: int
    change_count: int
    change_progress: float
...
@dataclass(frozen=True)
class ChangeBatchExecutionResult:
    change_ids: tuple[str, ...]
    results: tuple[ChangeExecutionResult, ...]
    ...
    @property
    def failed_result(self) -> ChangeExecutionResult | None:
...
BatchProgressCallback = Callable[[ChangeBatchProgressEvent], None] | None
