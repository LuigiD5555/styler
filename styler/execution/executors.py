...
from styler.package_managers.apt import reconcile_after_cancel, reconcile_dpkg
...
class PackageInstallExecutor(StepExecutor):
    ...
    def run(self, step: StepDefinition, ctx: ExecutionContext) -> StepResult:
        ...
        try:
            idle_timeout = max(5.0, float(step.config.get("idle_timeout_seconds", 300)))
        except (TypeError, ValueError):
            idle_timeout = 300.0
        command = run_step_command(
            ctx, step, argv, timeout=step.timeout, idle_timeout=idle_timeout,
        ...
        if command.cancelled:
            recovery = reconcile_after_cancel(command.command)
            recovery_message = recovery.message if recovery is not None else ""
            message = f"La instalación de {name} fue cancelada."
            if recovery_message:
                message += f" {recovery_message}"
            return StepResult(
                step.id,
                step.step_type,
                False,
                ...
                data={
                    ...
                    "manager": manager,
                    "package": name,
                    "dpkg_reconcile_needed": bool(recovery and recovery.needed),
                    "dpkg_repaired": bool(recovery and recovery.repaired),
                },
            )
        if command.timed_out:
            if command.timeout_reason == "idle":
                message = (
                    f"La instalación de {name} dejó de mostrar actividad durante "
                    f"{idle_timeout:g} segundos; Styler la detuvo como posible bloqueo."
                )
            else:
                message = f"La instalación de {name} excedió su límite máximo de seguridad."
            return StepResult(
                step.id,
                step.step_type,
                False,
                ...
                data={
                    "artifact": artifact,
                    "returncode": code,
                    "timeout_reason": command.timeout_reason,
    ...
    def _is_installed(manager: str, name: str) -> bool:
        ...
        if commands is None:
            return False
        result = ProcessRunner(timeout=20).run(commands, timeout=20)
        if manager == "apt":
...
class AptReconcileExecutor(StepExecutor):
    """Nodo semántico para auditar/reparar DPKG después de APT.

    Puede declararse con ``run_if: all_complete`` detrás de un command APT para
    que la reparación no forme parte de la supervisión genérica de procesos.
    """

    @property
    def step_type(self) -> str:
        return "apt_reconcile"

    def run(self, step: StepDefinition, ctx: ExecutionContext) -> StepResult:
        privilege = step.config.get("privilege") or admin_prefix() or []
        result = reconcile_dpkg(privilege=list(privilege))
        success = (not result.needed) or result.repaired
        status = Status.RECONCILED if success else Status.FAILED
        return StepResult(
            step.id,
            step.step_type,
            success,
