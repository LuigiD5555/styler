"""
styler.execution.processes
=======================
Frontera de Styler con procesos externos usados por operaciones de dominio.

`ProcessRunner` ejecuta comandos reales para operaciones de dominio que todavía
no se expresan como nodos nativos de PipeCraft. Las simulaciones viven sólo en tests.
"""
...
from styler.execution.commands import CommandExecutor
...
Runner = CommandExecutor  # alias de compatibilidad; nuevo código usa CommandExecutor.
...
class ProcessRunner:
    ...
    def run_streaming(
        ...
        *,
        ...
        # Un comando de instalación puede tardar mucho sin estar colgado. Si el
        # llamador declara ``idle_timeout``, la ausencia de actividad pasa a ser
        # la señal de bloqueo y dejamos de imponer el timeout total por defecto.
        # Un ``timeout`` explícito sigue funcionando como techo duro de seguridad.
        limit = timeout if timeout is not None else (None if idle_timeout is not None else self.timeout)
        started = time.monotonic()
        ...
        def read_pipe() -> None:
            try:
                ...
                while True:
                    # ``BufferedReader.read(4096)`` puede esperar a llenar el
                    # bloque y ocultar minutos de progreso. ``os.read`` entrega
                    # los bytes disponibles inmediatamente, incluidos \r de
                    # barras de progreso de apt/flatpak.
        ...
        try:
            while True:
                ...
                if chunk is None:
                    break
                if chunk is not ...:
                    # Cualquier byte recibido cuenta como progreso observable,
                    # incluso barras que actualizan con \r sin producir una línea.
        ...
        if cancelled or self._cancel_requested.is_set():
            return CommandResult(
                130, stdout, "La operación fue cancelada y su grupo de procesos fue detenido.",
        ...
        if timed_out:
            if timeout_reason == "idle" and idle_timeout is not None:
                detail = (
                    "La operación se detuvo porque no produjo actividad observable "
                    f"durante {idle_timeout:g} segundos."
                )
            else:
                detail = "La operación excedió el tiempo máximo permitido."
            return CommandResult(
                124, stdout, detail,
                command=tuple(argv), pid=process.pid,
                elapsed_seconds=max(0.0, time.monotonic() - started),
                timed_out=True, timeout_reason=timeout_reason,
            )
...
def run_step_command(
    ...
    *,
    ...
    # ``ProcessRunner`` no necesita shell y conserva stdin cerrado. ``env`` y
