...
from textual.widgets import (
    Button, Checkbox, Footer, Input, Label, ListItem, ListView,
...
from styler.changes import (
    AutomationLevel, ChangeCard, ChangeExecutionResult, ChangePlan,
    ChangeProgressEvent, ChangeService, ChangeStatus, ProviderOption,
)
...
class ChangeRow(ListItem):
    ...
    def __init__(self, card: ChangeCard, *, side: str) -> None:
        safe = "".join(ch if ch.isalnum() else "-" for ch in card.change_id)
        super().__init__(id=f"{side}-change-{safe}", classes=f"change-row {card.status}")
        self.card = card
...
class ChangesScreen(StylerScreen):
    ...
    async def refresh_changes(self) -> None:
        ...
        async with self._refresh_lock:
            ...
            available_rows = [
                ChangeRow(card, side="available")
                for card in self.app.changes.available_changes()
            ]
            if available_rows:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if button_id not in {"configure-change", "integrate-change", "delete-available-change", "remove-change"}:
