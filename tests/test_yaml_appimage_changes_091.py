...
def _ubuntu_change_service(tmp_path: Path) -> ChangeService:
    service = ChangeService(root=tmp_path / "library", home=tmp_path / "home")
    service._target = Target(distro_id="ubuntu", family="ubuntu", root=str(tmp_path / "library"))
    return service
...
def test_affinity_plan_composes_appimagelauncher_before_affinity(tmp_path, monkeypatch):
    ...
    service = _ubuntu_change_service(tmp_path)
