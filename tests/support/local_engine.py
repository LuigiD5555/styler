"""Scheduler en proceso exclusivo de tests para contratos de planificación."""
...
from tests.support.local_events import EventWriter
...
from styler.planning.plan import compile_workflow, plan_to_dict
from tests.support.local_scheduler import schedule
from styler.planning.selection import SelectionError, SelectionPreview, preview_selection, select_plan_nodes
...
def execute_backend(workflow, context, plan, registry):
    """Backend in-process exclusivo de la suite; nunca se distribuye."""
    del plan
    return WorkflowEngine(registry, backend="local").run(workflow, context)
