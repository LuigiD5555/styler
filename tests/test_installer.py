...
#!/usr/bin/env bash
if [[ "${1:-}" == "--version" ]]; then echo 'Styler 0.9.1'; exit 0; fi
exit 0
STYLER
...
def test_installer_activates_only_a_fully_verified_staged_installation(tmp_path):
    ...
    assert "0.9.1" in installed.stdout
...
def test_installer_process_itself_exports_user_bin_before_post_install_commands():
...
def test_installer_builds_from_sanitized_staging_copy_not_source_tree():
    ...
    # Los residuos que provocaron fallos anteriores deben excluirse de la copia.
...
def test_installer_creates_immediate_bridge_in_active_conda_path(tmp_path):
    ...
    assert "0.9.1" in resolved.stdout
